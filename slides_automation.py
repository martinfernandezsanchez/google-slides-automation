...
class GoogleSlidesAutomation:
    ...
    def _batch_update_with_tracking(self, presentation_id: str, requests: List[Dict[str, Any]], max_size_bytes: int, operation_description: str = "Unknown operation") -> None:
        """Execute batch update with size checking and tracking."""
        if not requests:
            self.logger.log_debug("Skipping empty batch update")
            return
    ...
    def _process_dynamic_tables(self, presentation_id: str, presentation: Dict, json_data: Dict[str, Any]) -> None:
        """
        Process dynamic table operations based on array data.
        
        This method handles:
        1. Tables with headers only that need to be populated with array data
        2. Slide duplication when array has more than 4 items (to keep tables under 6 rows)
        3. Slide deletion when array is empty
        """
        # Find all tables with array markers
        tables_with_markers = self.api_handler.find_tables_with_array_markers(presentation)
        self.logger.log_info(f"Found {len(tables_with_markers)} tables with array markers")
        ...
        for table_info in tables_with_markers:
            ...
            if array_key in json_data:
                ...
                if not array_data:  # Empty array - mark slide for deletion
                    slides_to_delete.append(table_info['slide_id'])
                else:
                    # Calculate how many slides we need
                    items_per_slide = 5  # Max 5 data rows per slide (plus 1 header row = 6 total)
                    total_slides_needed = (len(array_data) + items_per_slide - 1) // items_per_slide
        ...
        # Handle slide duplication first
        slide_duplication_requests = []
        for slide_info in slides_to_process:
            if slide_info['total_slides_needed'] > 1:
                # Duplicate the slide for additional pages
                for _ in range(slide_info['total_slides_needed'] - 1):
                    slide_duplication_requests.append(
                        self.api_handler.create_duplicate_slide_request(slide_info['slide_id'])
                    )
        
        # Execute slide duplications
        if slide_duplication_requests:
            self.api_handler.batch_update_with_size_check(presentation_id, slide_duplication_requests)
        
        # Delete slides marked for deletion
        if slides_to_delete:
            delete_requests = [
                self.api_handler.create_delete_slide_request(slide_id) 
                for slide_id in slides_to_delete
            ]
            self.api_handler.batch_update_with_size_check(presentation_id, delete_requests)
        
        # Second pass: populate tables with data
        for slide_info in slides_to_process:
            self._populate_table_with_array_data(presentation_id, slide_info)
    

    
    def _populate_table_with_array_data(self, presentation_id: str, slide_info: Dict) -> None:
        """
        (Deprecated) This method is no longer used for sending batch updates. Use _collect_table_population_requests instead.
        """
        pass
    
    def _process_erase_slides(self, presentation_id: str, presentation: Dict, json_data: Dict[str, Any]) -> None:
        """Process erase slide operations."""
        requests = []
        
        for slide in presentation.get('slides', []):
            slide_id = slide['objectId']
            
            # Check for erase slide identifiers in the slide
            for element in slide.get('pageElements', []):
                if 'shape' in element and 'text' in element['shape']:
                    text_elements = element['shape']['text'].get('textElements', [])
                    for text_element in text_elements:
                        if 'textRun' in text_element:
                            text = text_element['textRun'].get('content', '')
                            # Look for erase slide pattern: {{ERASE_SLIDE}}
                            if '{{ERASE_SLIDE}}' in text:
                                requests.append({
                                    'deleteObject': {
                                        'objectId': slide_id
                                    }
                                ...
                                break
                    if any('{{ERASE_SLIDE}}' in text_element.get('textRun', {}).get('content', '') 
                           for text_element in text_elements if 'textRun' in text_element):
                        break
        
        if requests:
            self.api_handler.batch_update_with_size_check(presentation_id, requests, operation_description="Delete slides with ERASE_SLIDE markers")
    
    def _process_duplicate_slides(self, presentation_id: str, presentation: Dict, json_data: Dict[str, Any]) -> None:
        """Process duplicate slide operations."""
        requests = []
        
        for slide in presentation.get('slides', []):
            slide_id = slide['objectId']
            
            # Check for duplicate slide identifiers
            for element in slide.get('pageElements', []):
                if 'shape' in element and 'text' in element['shape']:
                    text_elements = element['shape']['text'].get('textElements', [])
                    for text_element in text_elements:
                        if 'textRun' in text_element:
                            text = text_element['textRun'].get('content', '')
                            # Look for duplicate slide pattern: {{DUPLICATE_SLIDE:count}}
                            match = re.search(r'\{\{DUPLICATE_SLIDE:(\d+)\}\}', text)
                            if match:
                                ...
                                break
                    if any(re.search(r'\{\{DUPLICATE_SLIDE:\d+\}\}', text_element.get('textRun', {}).get('content', '')) 
                           for text_element in text_elements if 'textRun' in text_element):
                        break
        
        if requests:
            self.api_handler.batch_update_with_size_check(presentation_id, requests, operation_description="Duplicate slides with DUPLICATE_SLIDE markers")
    
    def _process_duplicate_table_rows(self, presentation_id: str, presentation: Dict, json_data: Dict[str, Any]) -> None:
        """Process duplicate table row operations."""
        requests = []
        ...
        for slide in presentation.get('slides', []):
            for element in slide.get('pageElements', []):
                if 'table' in element:
                    ...
                    # Check each cell for duplicate row identifiers
                        ...
                        for cell in row.get('tableCells', []):
                            if 'text' in cell:
                                ...
                                for text_element in text_elements:
                                    if 'textRun' in text_element:
                                        text = text_element['textRun'].get('content', '')
                                        # Look for duplicate row pattern: {{DUPLICATE_ROW:count}}
                                        match = re.search(r'\{\{DUPLICATE_ROW:(\d+)\}\}', text)
                                        if match:
                                            count = int(match.group(1))
                                            for _ in range(count):
                                                requests.append({
                                                    'duplicateObject': {
                                                        'objectId': f"{table_id}_row_{row_index}"
                                                    }
                                                })
                                            break
                                if any(re.search(r'\{\{DUPLICATE_ROW:\d+\}\}', text_element.get('textRun', {}).get('content', '')) 
                                       for text_element in text_elements if 'textRun' in text_element):
                                    break
        
        if requests:
            self.api_handler.batch_update_with_size_check(presentation_id, requests, operation_description="Duplicate table rows with DUPLICATE_ROW markers")
    
    def _process_replace_text(self, presentation_id: str, presentation: Dict, json_data: Dict[str, Any]) -> None:
        """Process text replacement operations."""
        requests = []
        ...
        if requests:
            self.api_handler.batch_update_with_size_check(presentation_id, requests, operation_description="Text replacement for all placeholders")
